...
- **scheduler.js** — Pure computation. `computeBookingSlots()` generates all needed tee time slots for the next N days based on the recurring schedule in config. When a schedule entry has `course: "alternating"`, it calls `resolveAlternatingCourse(dateStr)` to assign a concrete `'Pines'` or `'Oaks'` value per-date before inserting into the DB. `groupByDateAndTime()` groups pending DB rows for batch processing. Each consecutive slot is offset by 10 minutes (e.g., 12:00, 12:10, 12:20).
...
1. A single `SiteAutomation` session is created and authenticated.
2. `runSync(site)` is called first (syncs DB with site reservation history).
3. `new BookingEngine({ site }).run()` is called next (books any pending slots).
4. The shared session is closed in a `finally` block.
5. The next 06:00 fire is scheduled with `setTimeout`.
